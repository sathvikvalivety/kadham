...
async function verifyDeposit(req, res) {
  const { depositId, status, materialType, weightKg, ecoScore, remarks } = req.body;
  ...
  await WasteDeposit.updateWasteDepositVerification({
    ...
    materialType: materialType || "MIXED_RECYCLABLES",
    weightKg: weightKg || 1.0,
    ecoScore: ecoScore || 80
  });

  console.log(`[BACKEND] Verified Deposit #${depositId} via AI Service: ${status}`);
  ...
  return res.json({
    ...
    weightKg,
    ecoScore,
    remarks: remarks || "Verification complete."
  });
}
